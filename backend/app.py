...
def valider_pv(id_pv):
    """Valide le PV et met à jour le stock pour chaque ligne."""
    try:
        ...
        for l in lignes.data:
            ...
            nouveau_stock = stock_actuel + (l.get("quantite_livree_site") or 0)
...
@app.route("/api/select/bcs-valides", methods=["GET"])
@require_auth
def select_bcs_valides():
    """Retourne uniquement les BC validés pour le select du PV."""
    try:
        res = supabase.table("bon_commande") \
            .select("id_bon_commande, numero_bc, fournisseur_ami(raison_sociale)") \
...
def valider_pv(id_pv):
    ...
    try:
        ...
        # Mettre à jour le stock article par article
        for l in lignes.data:
            id_art = l.get("id_article")
            qte    = int(l.get("quantite_livree_site") or 0)
            if not id_art or qte <= 0:
                continue
            art = supabase.table("article") \
                .select("quantite_en_stock") \
                .eq("id_article", id_art).single().execute()
            nouvelle_qte = (art.data.get("quantite_en_stock") or 0) + qte
            supabase.table("article") \
                .update({"quantite_en_stock": nouvelle_qte}) \
                .eq("id_article", id_art).execute()
